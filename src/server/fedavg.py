...
class FedAvgServer:
    ...
    def __init__(self, args: DictConfig, init_trainer=True, init_model=True):
        ...
        # BEAT 实验新指标
        self.round_time_sec: list[float] = []
        self.round_upload_bytes: list[float] = []
    ...
    def train(self):
        ...
        for E in self.train_progress_bar:
            ...
            # )
            # BEAT 实验新增 / 注释
            self.round_time_sec.append(end - begin)
            avg_round_time = float(np.mean(self.round_time_sec))
    ...
    def save_metrics_stats(self):
        ...
        # >>> 新增：把每轮耗时 & 上传字节拼进去
        num_rounds = len(self.round_time_sec)
        if df.empty:
            # 建一个空表架子：index 从 1..num_rounds
            df.index = np.arange(1, num_rounds + 1)
            df.index.name = "epoch"

        # 截断长度对齐，避免越界
        rt = np.array(self.round_time_sec)[: len(df)]
        ub = (np.array(self.round_upload_bytes)[: len(df)] / 1024.0 / 1024.0)  # 转 MB

        # 不重复插入列
        # round_time_sec = 每轮端到端训练时间；upload_megabytes = 本轮平均每个被选客户端上传大小（MB）
        if "round_time_sec" not in df.columns:
            df.insert(df.shape[1], "round_time_sec", rt)
        if "upload_megabytes" not in df.columns:
            df.insert(df.shape[1], "upload_megabytes", ub)
