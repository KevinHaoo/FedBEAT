...
class FedAvgClient:
    ...
    def package(self):
        ...
        # >>> 新增：估算上传字节（默认以“常规参数”作为上传载荷；return_diff 场景会在下面替换）
        def _nbytes(sd: dict[str, torch.Tensor]) -> int:
            return int(sum(t.numel() * t.element_size() for t in sd.values()))

        client_package["upload_bytes"] = _nbytes(client_package["regular_model_params"])
        ...
        if self.return_diff:
            ...
            # return_diff 时，真正上传的是 diff；同步修正统计值并移除 regular
            client_package["upload_bytes"] = _nbytes(client_package["model_params_diff"])
